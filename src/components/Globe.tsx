...
import { Globe as GlobeIcon, Map as MapIcon, Compass } from 'lucide-react';
...
const getSatIcon = (color: string) => createCustomIcon(`
  <div class="relative flex items-center justify-center">
    <span class="animate-pulse absolute inline-flex h-5 w-5 rounded-full" style="background-color: ${color || '#10b981'}; opacity: 0.2"></span>
    <span class="relative inline-flex rounded-full h-3.5 w-3.5 border-1.5 border-white shadow-[0_0_6px_${color || '#10b981'}]" style="background-color: ${color || '#10b981'}"></span>
  </div>
`, 'custom-sat-pin', [20, 20]);
...
function MapViewController({ flyToCoords }: { flyToCoords: [number, number] | null }) {
  const map = useMap();
  useEffect(() => {
    if (flyToCoords && typeof flyToCoords[0] === 'number' && typeof flyToCoords[1] === 'number' && !isNaN(flyToCoords[0]) && !isNaN(flyToCoords[1])) {
      map.flyTo(flyToCoords, 5, {
        animate: true,
        duration: 1.2,
        easeLinearity: 0.25
      });
...
function CenterTracker({ setViewCenter }: { setViewCenter: (center: [number, number]) => void }) {
  const map = useMap();
  useMapEvents({
    move() {
...
export default function Globe({ observer, iss, satellites, onSelectCoordinates, currentTime, selectedObjectId, timeMultiplier }: GlobeProps) {
  ...
  const futurePathSegments = useMemo(() => {
    try {
      const rawPath = getIssFuturePath(currentTime);
  ...
  }, [selectedObjectId, satellites, currentTime]);
  ...
  useEffect(() => {
    if (observer && typeof observer.latitude === 'number' && typeof observer.longitude === 'number' && !isNaN(observer.latitude) && !isNaN(observer.longitude)) {
      setFlyToCoords([observer.latitude, observer.longitude]);
    }
    ...
    <div className="bg-slate-950/40 border border-slate-800/60 rounded-xl p-5 shadow-2xl h-full flex flex-col justify-between backdrop-blur-md" id="earth-globe-panel">
      ...
      <div className="flex items-center justify-between mb-4 border-b border-slate-900 pb-2">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xs font-bold uppercase tracking-widest text-slate-500 font-serif">Geocentric Tracker</h2>
        </div>
        <div className="flex items-center gap-1.5">
          <button
            onClick={handleFocusStation}
            className="p-1 px-2.5 rounded-lg border border-slate-800 bg-slate-1000 text-[10px] text-slate-400 hover:text-white transition flex items-center gap-1 font-mono hover:border-indigo-500/50 cursor-pointer"
          ...
          <button
            ...
            id="snap-chennai-btn"
          >
            <GlobeIcon className="w-3 h-3 text-emerald-400" />
            <span>CHENNAI</span>
          </button>
        </div>
      </div>
      ...
      <div className="relative flex-1 min-h-[300px] w-full rounded-xl overflow-hidden border border-slate-800/60 bg-slate-950/90 shadow-inner" id="leaflet-map-wrapper">
        ...
        <MapContainer
          ...
          className="z-10"
        >
          {/* Esri World Dark Gray Base Map & Reference Labels in English */}
          <TileLayer
          ...
          {/* Map view center tracker */}
          <CenterTracker setViewCenter={setViewCenter} />
        ...
        <div className="absolute bottom-2 left-2 z-[1000] text-[9px] font-mono text-slate-400 bg-slate-950/80 px-2 py-1 rounded-md border border-slate-800/80 pointer-events-none uppercase">
          Click map area to relocate Ground Station
        </div>
      </div>

      {/* Grid Coordinates Footer Info */}
      <div className="bg-slate-900/40 border border-slate-850 rounded-xl p-2 px-3 flex items-center justify-between text-[11px] font-mono text-slate-400 mt-2">
