...
export default function App() {
  ...
  const questList = useMemo(() => ["urmajor", "orion", "cassiopeia", "cygnus", "leo"], []);
  ...
  const executeLocalBackupTelemetry = (time: number = activeTime) => {
    ...
    // Mock Satellites
    const mockSats = [
      { id: "hst", name: "Hubble Space Telescope", color: "#93C5FD", lat: 20, lng: -140, magnitude: 1.5 },
      { id: "noaa-19", name: "NOAA-19 Weather", color: "#FDE68A", lat: -35, lng: 120, magnitude: 3.5 }
    ].map(s => ({
      ...s,
      type: "satellite" as const,
      size: 4,
      coordinates: { latitude: s.lat, longitude: s.lng },
      localCoordinates: { altitude: s.lat > 0 ? 32 : -45, azimuth: 120, rangeKm: 850 },
      description: "Low orbit instrumentation satellite.",
  ...
  const handleAddNewLog = (object: CelestialObject, logContent: string, genType: 'ai' | 'static_fallback' | 'intel') => {
    const newLog: ObsLog = {
      id: `log-${Date.now()}`,
  ...
  return (
    <div className={`min-h-screen flex flex-col justify-between font-sans selection:bg-indigo-500/30 selection:text-white ${nightVision ? 'night-vision' : ''}`} id="zenith-app-root">
      ...
      <header className="border-b border-white/10 bg-slate-950/55 backdrop-blur-xl px-6 md:px-8 lg:px-12 py-3.5 sticky top-0 z-50 shadow-[inset_0_1px_1px_rgba(255,255,255,0.07),0_10px_30px_-10px_rgba(0,0,0,0.5)]">
        <div className="max-w-[1920px] mx-auto w-full flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-mono font-bold bg-indigo-950/80 text-indigo-400 border border-indigo-500/25 px-1.5 py-0.5 rounded uppercase">The Celestial Eye v1.4</span>
              </div>
              <p className="text-[10px] font-mono tracking-wider text-slate-500 uppercase mt-0.5">Horizontal Satellite & Planet Mapping Interface</p>
            </div>
          </div>
          ...
          {bortleScale >= 5 && (
            <div className="flex items-center gap-3 bg-amber-950/40 border border-amber-500/30 rounded-xl p-2 px-4 shadow-[0_0_12px_rgba(245,158,11,0.06)] animate-pulse" id="dark-sky-alert-prompt">
              ...
              <div className="flex flex-col text-left">
                <span className="text-[9px] font-bold text-amber-400 font-mono tracking-wider uppercase leading-none">DARK-SKY ALERT</span>
                <span className="text-[10px] text-amber-200 font-mono mt-1">
              ...
              <button
                ...
                className="ml-2 p-1 px-2.5 rounded bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold font-mono text-[9px] uppercase tracking-wider cursor-pointer shadow-md transition-all active:scale-95 shrink-0"
              >
                Shift Coordinates
              </button>
            </div>
          ...
          <div className="hidden lg:flex items-center gap-4 bg-white/[0.03] border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.04)] backdrop-blur-md rounded-xl p-2 px-4" id="header-signal-station">
            <div className="flex flex-col text-left">
              <span className="text-[10px] font-semibold text-slate-500 font-mono block leading-none uppercase">NODE LOCATORS</span>
              <span className="text-[11px] font-bold text-slate-300 mt-1 block font-mono">
            ...
            <div className="flex items-center gap-2.5">
              <div className="flex flex-col text-right">
                <span className="text-[9px] font-bold text-slate-500 font-mono block leading-none">TELEMETRY LINK</span>
                <span className={`text-[10px] font-bold font-mono mt-1 block uppercase leading-none ${
          ...
          >
            ...
            <span className="text-[10px] font-bold font-mono uppercase tracking-wider">NIGHT VISION: {nightVision ? 'ON' : 'OFF'}</span>
          </button>
          ...
          <div className="flex items-center gap-4 bg-white/[0.03] border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.04)] backdrop-blur-md rounded-xl p-2 px-4" id="simulator-chronostat">
            <div className="flex items-center gap-2">
              ...
              <div className="text-right">
                <span className="text-[10px] font-semibold text-slate-500 font-mono block leading-none uppercase">STATION COORD TIME</span>
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                <span className="text-[9px] font-bold text-amber-500 font-mono block leading-none">☀ RISE</span>
                <span className="text-[10px] font-semibold text-slate-300 mt-1 block font-mono">
                  {sunrise}
                </span>
              </div>
              <div className="h-4 w-[1px] bg-slate-800/60" />
              <div className="text-right">
                <span className="text-[9px] font-bold text-indigo-400 font-mono block leading-none">☽ SET</span>
                <span className="text-[10px] font-semibold text-slate-300 mt-1 block font-mono">
                  {sunset}
                </span>
              </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                id="fast-sim-btn-2"
              >
                <span className="text-[10px] font-bold font-mono px-0.5">WARP</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {cloudCover !== null && cloudCover > 60 && (
        <div className="bg-red-950/40 border-b border-red-500/20 px-6 md:px-8 lg:px-12 py-2.5 text-red-500 font-mono text-xs transition-all" id="weather-cloud-warning">
          <div className="max-w-[1920px] mx-auto w-full flex items-center justify-between">
            ...
            <span className="text-[10px] opacity-75 hidden sm:inline-block">CLOUD COVER: {cloudCover}%</span>
          </div>
        </div>
      )}
          ...
          <div className="lg:col-span-4 flex flex-col bg-slate-950/40 border border-slate-800/60 rounded-xl p-5 shadow-2xl backdrop-blur-md" id="stargazing-quest-hub">
            <div className="flex items-center justify-between border-b border-slate-900 pb-2 mb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xs font-bold uppercase tracking-widest text-slate-300 font-mono">Observation Command</h2>
              </div>
              <div className="text-[10px] text-emerald-400 font-mono font-bold uppercase tracking-wider bg-emerald-950/30 px-2 py-0.5 rounded border border-emerald-900/30 animate-pulse">
                XP: {questScore}
              </div>
            </div>
      ...
      <div className="bg-slate-950 border-t border-slate-900/60 px-6 md:px-8 lg:px-12 py-4 select-none" id="temporal-warp-panel">
        <div className="max-w-[1920px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-mono text-indigo-400 font-bold uppercase tracking-wider block">TEMPORAL WARP HARNESS</span>
              <h3 className="text-xs font-mono font-extrabold text-slate-100 uppercase tracking-tight">Sky Shift: {timeOffsetHours === 0 ? "REAL-TIME" : `${timeOffsetHours > 0 ? '+' : ''}${timeOffsetHours.toFixed(1)} HOURS`}</h3>
            </div>
          </div>

          <div className="flex-1 w-full max-w-2xl px-4 flex flex-col gap-1.5">
            <div className="flex items-center justify-between text-[9px] font-mono text-slate-500 font-bold uppercase tracking-widest px-1">
              <span>-12h (PAST)</span>
              <span>-6h</span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded font-mono text-[10px] font-extrabold transition cursor-pointer border ${
                timeOffsetHours === 0
      ...
      {/* 3. Footer credits */}
      <footer className="h-12 bg-slate-950 border-t border-slate-800/50 px-6 md:px-8 lg:px-12 text-[10px] tracking-widest text-[#64748b] uppercase font-bold">
